...
usuarios = {
    "EEL CIBERSEGURIDAD": {"password": "1234", "documento": 1001},
    "CLIENTE B": {"password": "5678", "documento": 1002}
}
...
PANEL = """
<h1>Panel privado</h1>
<p>Usuario autenticado: <strong>{{ usuario }}</strong></p>
<button onclick="verDocumento()">VER MI DOCUMENTO</button>
<br><br>
...
def api_documento():
    ...
    if doc_id not in documentos:
        return jsonify({"error": "Documento inexistente"}), 404

    # Vulnerabilidad IDOR intencional:
    # se verifica la sesión, pero NO que el documento
